...
export default {
  ...
  async fetch() {
    ...
    let wallet = await WalletServices.getAllWalletList();
    this.bankList = data;
    wallet.data.forEach(item => {
      if (item['wallet_slug'] === "connect-ips") {
  ...
  created() {
    ...
    this.isFromBusinessPlan = this.$store.getters['sharedUser/GET_IS_FROM_BUSINESS_PLAN'];

    if (this.isFromBusinessPlan) {
      const {start_range, end_range} = this.$store.getters['sharedUser/GET_BUSINESS_PLAN_AMOUNT'];
      this.priceStartRange = start_range;
      this.priceEndRange = end_range;
    }
  ...
  methods: {
    ...
    saveOfflinePayment() {
      ...
      data.append('wallet_code', this.walletInfo['wallet_code']);
      data.append('payment_partner', this.walletInfo['wallet_name']);
      ...
      data.append('branch_name', this.receiverInformation.branchName);
      ...
      data.append('sender_branch_name', this.senderInformation.branchName);
