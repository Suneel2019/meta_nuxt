<template>
  <v-container class="maintenance-mode fill-height">
    ...
    <v-row no-gutters justify="space-between" align="center" v-else>
      <v-col cols="5">
        <v-row no-gutters>
          <v-col cols="12">
            <img src="~/assets/img/shared/maintenance.webp">
          </v-col>
      ...
      <v-col cols="6" class="body-2">
        <v-row no-gutters justify="center" class="text-center">
          <v-col cols="12" class="headline">Code: 503</v-col>
          <v-col cols="12" class="headline">We are in maintenance mode.</v-col>
          <v-col cols="12" class="mt-3">
            AllPasal is currently down for maintenance. We expect to be back in couple of hours. <br> Thanks for your
            patience.
          </v-col>
...
export default {
  ...
  methods: {
    ...
    checkMaintenanceMode() {
      ...
      this.$store.dispatch("error/checkMaintenanceMode").then(res => {
        ...
        if (!res['maintenance_mode']) {
          ...
          this.$store.commit("error/SET_MAINTENANCE_CODE", res['maintenance_mode']);
