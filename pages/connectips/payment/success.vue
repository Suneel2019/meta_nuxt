      ...
      <v-col cols="12">
        <v-row v-if="transactionAmount" no-gutters align="center" justify="center" style="height: 150px;">
          <v-col cols="5" xl="3" lg="3">
            <v-icon color="green" dark size="150">mdi-check-circle</v-icon>
          </v-col>
        </v-row>
        <v-row v-if="transactionAmount" no-gutters align="center" justify="center" class="mt-3 ml-7">
          <v-col cols="12" xl="5" lg="5">
            <div class="load_balance_successful_message green--text">Your payment was successful.</div>
          </v-col>
        </v-row>
        ...
        <v-row v-if="transactionAmount" no-gutters align="center" justify="center">
          <v-col cols="10" xl="4" lg="4">
            <div class="transaction_id_text black--text mt-5 ml-3">Transaction Amount : Rs. {{
                transactionAmount
              }}
            </div>
          </v-col>
...
export default {
  name: "success",
      ...
      '$routes': {
        ...
        handler() {
          ...
          if (this.$route.query.TXNID) {
            ...
            this.successTransactionId = this.$route.query.TXNID;

            this.checkWhetherTheTransactionIsSuccessful(this.successTransactionId);
    ...
    methods: {
      ...
      checkWhetherTheTransactionIsSuccessful(transactionId) {

        this.$store.dispatch("connectIPSStore/checkForTheTransactionSuccess", {TXNID: transactionId}).then(response => {
